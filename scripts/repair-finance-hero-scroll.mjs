#!/usr/bin/env node
/**
 * Remove command-scroll divs wrongly injected inside AixiaHero blocks,
 * then balance scroll wrappers per FinancePage block.
 */
...
function stripScrollInsideHero(text) {
...
function fixFile(text) {
  text = stripScrollInsideHero(text);
  text = text.replace(
