...
export default function AIActivityLogsPage() {
  ...
  const metrics = useMemo(() => {
    ...
    const settingsChanges = logs.filter((log) =>
      log.entity_type.includes("setting")
    ).length;
    const cacheActions = logs.filter((log) =>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
                    Review AI settings changes, cache actions, promotions, guardrail changes, feedback events, and system audit history.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-3 border-b border-white/10 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">
                Latest 200 AI admin activity records.
              </p>
            </div>
            ...
            {loading ? (
              ...
              <div className="divide-y divide-white/5">
                {filteredLogs.map((log) => (
                  <div
                    key={log.id}
                    className="grid gap-4 px-5 py-4 transition hover:bg-white/[0.025] lg:grid-cols-[220px_220px_1fr_180px]"
                  >
                    <div>
