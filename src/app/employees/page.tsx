...
export default function EmployeesPage() {
  ...
  const approveUser = async (userId: string) => {
    ...
    try {
      const { error: updateError } = await supabase
        .from("profiles")
        .update({ status: "active", role: roleToApply, updated_at: nextUpdatedAt })
        .eq("user_id", userId);
      ...
      setProfiles((prev) =>
        prev.map((profile) =>
          profile.user_id === userId ? { ...profile, status: "active", role: roleToApply, updated_at: nextUpdatedAt } : profile
        )
      );
    } catch (err) {
  ...
  const rejectUser = async (userId: string) => {
    ...
    try {
      const { error: updateError } = await supabase
        .from("profiles")
        .update({ status: "rejected", updated_at: nextUpdatedAt })
        .eq("user_id", userId);
      ...
      setProfiles((prev) =>
        prev.map((profile) =>
          profile.user_id === userId ? { ...profile, status: "rejected", updated_at: nextUpdatedAt } : profile
        )
      );
    } catch (err) {
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 flex-wrap mb-3">
                    ...
                    {!user.profile_completed && user.status === "active" && (
                      <Badge className="bg-amber-500/20 text-amber-400 border-amber-500/30">PROFILE INCOMPLETE</Badge>
                    )}
                  </div>
