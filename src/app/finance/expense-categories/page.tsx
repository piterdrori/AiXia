...
export default function FinanceExpenseCategoriesPage() {
  ...
  async function loadPage() {
    try {
      ...
      setExpenseCategories((data || []) as FinanceExpenseCategoryRow[]);
    } catch (error) {
  ...
  const canCreateExpenseCategories = !!permissions?.createFinanceRecords;

  async function handleCreateExpenseCategory() {
    if (!canCreateExpenseCategories) {
    ...
    if (!trimmedName) {
      setSaveError("Expense category name is required");
      return;
    }
    ...
    try {
      ...
      setExpenseCategories(
        (prev) => [data as FinanceExpenseCategoryRow, ...prev]
      );
      setName("");
      setDescription("");
    } catch (error) {
      ...
      setSaveError(
        error instanceof Error ? error.message : "Failed to create expense category"
      );
    } finally {
      setCreating(false);
    }
    ...
    <div className="h-full flex flex-col gap-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage finance expense categories.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            variant="outline"
            onClick={() => navigate("/finance/revenue-categories")}
            className="border-border bg-background/40 text-white hover:bg-background/60"
          >
            Revenue Categories
          </Button>
      ...
      {canCreateExpenseCategories ? (
        <div className="border border-border rounded-xl p-4 bg-background/40">
          <div className="flex flex-col gap-3">
            <div className="text-white font-medium">Create Expense Category</div>
            ...
            <div className="flex justify-start">
              <Button
                onClick={() => void handleCreateExpenseCategory()}
                disabled={creating}
                className="text-white"
              >
                {creating ? "Creating..." : "Create Expense Category"}
              </Button>
            </div>
          </div>
        </div>
        ...
        <div className="border border-border rounded-xl p-4 bg-background/40 text-sm text-muted-foreground">
          You can view expense categories, but you do not have permission to create finance records.
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto pb-4 border border-border rounded-xl p-4">
        {loading ? (
          ...
          <div className="text-muted-foreground">No expense categories yet</div>
        ) : (
          <div className="flex flex-col gap-3">
