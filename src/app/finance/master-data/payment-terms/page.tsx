...
export default function FinancePaymentTermsPage() {
  ...
  async function handleArchiveToggle(row: FinancePaymentTermRow) {
    try {
      ...
      console.error("Failed to update payment term status:", actionError);
    }
  }
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="mt-2 max-w-3xl text-sm leading-7 text-white/55">
                  Define due-date standards like immediate payment, Net 15, Net 30, and other
                  commercial terms used across invoices and bills.
                </p>
              </div>
            </div>
            ...
            <table className="w-full min-w-[980px]">
              <thead>
                <tr className="border-b border-white/8 text-left">
                  {["Code", "Name", "Due Days", "Status", "Default", "Updated", "Actions"].map(
                    (label) => (
                      <th
                        key={label}
                ...
                {loading ? (
                  ...
                  filteredRows.map((row) => (
                    <tr key={row.id} className="border-b border-white/6 last:border-b-0">
                      ...
                      <td className="px-5 py-4 text-sm text-white">{row.due_days}</td>
        ...
        <DialogContent className="border-white/10 bg-[#0f1726] text-white sm:max-w-[620px]">
          <DialogHeader>
            ...
            <DialogDescription className="text-white/45">
              Standardize due-date behavior so invoices and bills use controlled commercial terms.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              <Input
                ...
                step="1"
                value={form.due_days}
                onChange={(e) => setForm((p) => ({ ...p, due_days: e.target.value }))}
              ...
              <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-black/15 px-4">
                <input
                  ...
                  className="h-4 w-4"
                />
                <label htmlFor="payment-term-default" className="text-sm text-white/80">
                  Set as default term
                </label>
              </div>
            </div>
