...
export default function FinanceShippingTermsPage() {
  ...
  const loadPage = useCallback(async () => {
    ...
    try {
      ...
      setRows(await getShippingTerms());
    } catch (loadError) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setError(saveError instanceof Error ? saveError.message : "Failed to save shipping term.");
    } finally {
      setSaving(false);
    }
  }

  async function handleArchiveToggle(row: FinanceShippingTermRow) {
    try {
      ...
      console.error("Failed to update shipping term status:", actionError);
    }
  }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-y-auto overflow-x-hidden">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="mt-2 max-w-3xl text-sm leading-7 text-white/55">
                  Define standardized delivery and shipment terms for future invoices, bills,
                  logistics references, and commercial documents.
                </p>
              </div>
            </div>
            ...
            <table className="w-full min-w-[1080px]">
              <thead>
                <tr className="border-b border-white/8 text-left">
                  {["Code", "Name", "Description", "Status", "Default", "Updated", "Actions"].map(
                    (label) => (
                      <th
                        key={label}
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan={7} className="px-5 py-10 text-sm text-white/50">
                      Loading shipping terms...
                    </td>
                  </tr>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="border-white/10 bg-[#0f1726] text-white sm:max-w-[620px]">
          <DialogHeader>
            <DialogTitle>
              {editingRow ? "Edit Shipping Term" : "Create Shipping Term"}
            </DialogTitle>
            <DialogDescription className="text-white/45">
              Keep shipment and delivery standards controlled for future finance and logistics flows.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4">
            ...
            <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-black/15 px-4 py-3">
              <input
                id="shipping-term-default"
                type="checkbox"
                checked={form.is_default}
                onChange={(e) => setForm((p) => ({ ...p, is_default: e.target.checked }))}
                className="h-4 w-4"
              />
              <label htmlFor="shipping-term-default" className="text-sm text-white/80">
                Set as default shipping term
              </label>
            </div>
          ...
          <DialogFooter>
            ...
            >
              {saving ? "Saving..." : editingRow ? "Save Changes" : "Create Shipping Term"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
