...
export default function FinanceTaxCodesPage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setError(saveError instanceof Error ? saveError.message : "Failed to save tax code.");
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="flex h-full min-h-0 flex-col overflow-y-auto overflow-x-hidden">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="mt-2 max-w-3xl text-sm leading-7 text-white/55">
                  Define standard tax rates and tax treatments used across invoices, bills,
                  items, and other finance records.
                </p>
              </div>
            </div>
            ...
            <table className="w-full min-w-[1080px]">
              <thead>
                <tr className="border-b border-white/8 text-left">
                  {["Code", "Name", "Rate", "Status", "Default", "Updated", "Actions"].map(
                    (label) => (
                      <th
                        key={label}
                  ...
                  filteredRows.map((row) => (
                    <tr key={row.id} className="border-b border-white/6 last:border-b-0">
                      ...
                      <td className="px-5 py-4 text-sm text-white/55">
                        {formatRateLabel(row.rate_percent)}
                      </td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="border-white/10 bg-[#0f1726] text-white sm:max-w-[620px]">
          <DialogHeader>
            <DialogTitle>
              {editingRow ? "Edit Tax Code" : "Create Tax Code"}
            </DialogTitle>
            <DialogDescription className="text-white/45">
              Standardize tax handling so finance records use controlled tax rates and labels.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              <Input
                ...
                onChange={(e) => setForm((p) => ({ ...p, rate_percent: e.target.value }))}
              ...
              <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-black/15 px-4 py-3">
                <input
                  ...
                  className="h-4 w-4"
                />
                <label htmlFor="tax-code-default" className="text-sm text-white/80">
