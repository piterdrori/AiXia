...
export default function FinanceExpenseFundingBatchDetailPage() {
  ...
  const fundingDetailRefreshTables = useMemo(
    () =>
      batchId
        ? [
            { table: "finance_expense_funding_batches" as const, filter: `id=eq.${batchId}` },
            { table: "finance_record_attachments" as const, filter: `entity_id=eq.${batchId}` },
            {
              table: "finance_payments_made" as const,
  ...
  useExpenseModuleRefresh({
    channelName: `finance-expense-funding-pool-detail-${batchId ?? "detail"}`,
    enabled: Boolean(batchId),
    tables: fundingDetailRefreshTables,
    onRefresh: () => void loadBatch("silent"),
  });
  ...
  const openExpensePaymentsTool = useCallback(() => {
    ...
    navigate(
      `/finance/transactions/expense-payments/new?source=batch&batchId=${batch.id}`
    );
      ...
      <AixiaNotFoundState
        ...
        action={
          <AixiaButton
            type="button"
            variant="secondary"
            onClick={() => navigate("/finance/transactions/expense-funding")}
          >
            Payment Control
          </AixiaButton>
        }
