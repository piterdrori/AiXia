...
import { Archive, Eye, FolderArchive, Receipt, RotateCcw, ShieldCheck, Trash2, WalletCards } from "lucide-react";
...
type EnrichedPaymentMade = PaymentMadeRow & {
  ...
  soleLinkedExpenseId?: string;
  recipientIdentity: FinanceEmployeeIdentity | null;
  recordType: "payment_made";
};

type ExpenseListContext = "default" | "waiting_confirmation";
...
export default function FinanceExpensesPaymentsMadePage() {
  ...
  const enrichedPayments = useMemo<EnrichedPaymentMade[]>(() => {
    return payments.map((payment) => {
      ...
      return {
        ...
        soleLinkedExpenseId:
          linkedAllocations.length === 1 ? linkedAllocations[0]?.expense_id : undefined,
        recipientIdentity,
      };
    });
  ...
  const payExpenseRows = useMemo(
    () => filterExpensePaymentTab(activePaymentRows, "pay_expenses"),
    [activePaymentRows],
  );

  const waitingConfirmationPayments = useMemo(
    () => filterExpensePaymentTab(activePaymentRows, "waiting_confirmation"),
    [activePaymentRows],
  );

  const recipientTrackingRows = useMemo(
    () => filterRecipientTrackingRows(activeExpenseRows),
    [activeExpenseRows],
  );
  ...
  const renderExpenseRows = useCallback(
    (
      ...
      mode: "active" | "archive",
      listContext: ExpenseListContext = "default"
    ) => {
      if (rows.length === 0) {
        return (
          <tr>
      ...
      return rows.map((expense) => {
        ...
        const isWaitingConfirmationContext = listContext === "waiting_confirmation";
        const primaryRoute = isWaitingConfirmationContext ? expenseRoute : reviewRoute;
        ...
        return (
          <tr key={expense.id} className="aixia-table-row">
            ...
            <AixiaTableActionsCell>
              ...
              {mode === "active" && isWaitingConfirmationContext ? (
                <AixiaButton
                  type="button"
                  variant="secondary"
                  disabled={rowActionDisabled}
    ...
    (
      ...
      return (
        <AixiaTableShell variant={mode === "archive" ? "archive" : "registry"}>
          ...
          <tbody>{renderExpenseRows(rows, mode, listContext)}</tbody>
        </AixiaTableShell>
      );
    },
    [renderExpenseRows]
  ...
  const renderExecutionRows = useCallback(
    (
      ...
      return rows.map((record) => {
        ...
        const isWaitingConfirmationContext = listContext === "waiting_confirmation";
        const soleLinkedExpenseId =
          !isBatch && record.recordType === "payment_made"
            ? record.soleLinkedExpenseId
            : undefined;
        const expenseRoute = soleLinkedExpenseId
          ? `/finance/transactions/expenses/${soleLinkedExpenseId}`
          : null;
        ...
        return (
          <tr key={`${record.recordType}-${record.id}`} className="aixia-table-row">
            ...
            <AixiaTableActionsCell>
              {isWaitingConfirmationContext && !isBatch ? (
                <>
                  <AixiaButton
                    type="button"
                    variant="primary"
    ...
    (
      ...
      return (
        <AixiaTableShell variant={mode === "archive" ? "archive" : "registry"}>
          ...
          <tbody>{renderExecutionRows(rows, mode, listContext)}</tbody>
        </AixiaTableShell>
      );
    },
    [renderExecutionRows]
    ...
    () => [
      {
        key: "pay-expenses",
        label: "Pay expenses",
        value: metrics.payExpenses.toLocaleString(),
        detail: "Active expense payments ready to manage.",
        tone: "cyan" as const,
      },
      {
        key: "waiting",
        label: "Waiting for confirmation",
        value: metrics.waitingConfirmation.toLocaleString(),
        detail: "Payments and expenses awaiting recipient confirmation.",
        tone: "amber" as const,
      },
    ],
    [metrics.payExpenses, metrics.waitingConfirmation]
  );
...
return (
    <FinancePage>
      <AixiaHero
        ...
        gradientTitle="Use Allocated Funds"
        title=""
        subtitle="Disburse from funding pools and track recipient confirmation"
      />
      ...
      <div className="aixia-command-scroll">
        ...
        description="Payment execution and allocation monitoring for funded employee expenses."
        icon={ShieldCheck}
      >
        Payment distributions and allocations use protected backend lifecycle RPCs.
      </AixiaAccessRule>
      ...
      >
        <AixiaRegistryToolbar
          search={
            <AixiaSearchField
              ...
              placeholder="Search payments or recipient confirmation..."
            />
          }
          secondaryActions={
            <AixiaButton
          ...
          primaryAction={
            <AixiaButton
              type="button"
              variant="primary"
              onClick={() => navigate("/finance/transactions/expense-payments/new")}
            >
              <WalletCards className="h-4 w-4" />
              Pay expenses
            </AixiaButton>
        ...
        {activeTab === "waiting_confirmation" ? (
          waitingConfirmationPayments.length > 0 ? (
            renderExecutionTable(waitingConfirmationPayments, "active", "waiting_confirmation")
          ) : (
            renderExpenseTable(recipientTrackingRows, "active", "waiting_confirmation")
          )
        ) : null}
      </AixiaSection>
      ...
      <AixiaSection
        title="Advanced"
        description="Allocation registry and other backend linkage tools."
        icon={FolderArchive}
      >
        <div className="aixia-stack">
          ...
          >
            {advancedOpen ? "Hide allocations registry" : "Show allocations registry"}
          </AixiaButton>
