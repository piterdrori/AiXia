      ...
      <CardContent className="space-y-5">
        <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-4">
          <div className="mb-2">
            ...
            <p className="text-xs text-slate-500">{t("taskDetail.discussion.addUpdateHelper")}</p>
          </div>
                ...
                mentionCandidates.map((profile) => (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs text-slate-500">{profile.role.toUpperCase()}</div>
                    </div>
                  </button>
                ))
              )}
          ...
          <div className="mt-3 flex items-center justify-between gap-3">
            <p className="text-xs text-slate-500">{t("taskDetail.discussion.visibilityNote")}</p>
        ...
        <div className="space-y-4">
          {comments.length === 0 ? (
            <div className="rounded-xl border border-dashed border-slate-800 bg-slate-950/40 p-8 text-center">
              ...
              <p className="mt-1 text-sm text-slate-500">{t("taskDetail.discussion.emptyDescription")}</p>
            </div>
          ) : (
            visibleComments.map((comment) => {
              const isMine = comment.user_id === currentUserId;
              const authorName = getProfileName(comment.user_id, profiles, t("taskDetail.fallbacks.unknown"));
              const authorRole = getProfileRole(comment.user_id, profiles);
              ...
              return (
                <div
                  ...
                  className={`rounded-xl border p-4 ${
                    isMine ? "border-indigo-800/40 bg-indigo-950/20" : "border-slate-800 bg-slate-950/50"
                  }`}
                >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="mt-1 flex items-center gap-1 text-xs text-slate-500">
                          ...
                          <span>{format(clock.shiftDate(comment.created_at), "MMM d, yyyy • h:mm a")}</span>
                        </div>
                      </div>
                    </div>
