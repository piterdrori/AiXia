...
export function AixiaPage({
  ...
  const rootClassName = [`aixia-page`, className].filter(Boolean).join(" ");

  if (surface === "command") {
    const commandClassName = [`aixia-dash-page`, `aixia-dash-page--command`, className]
      .filter(Boolean)
      .join(" ");

    return (
      <div className={commandClassName} data-page-surface="command">
        ...
        <div className={`aixia-dash-3d-stack ${scrollClassName}`.trim()} data-page-stack="command">
          {children}
        </div>
      </div>
    );
  ...
  return (
    <div className={rootClassName} data-page-surface="default">
